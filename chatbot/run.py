...
def inference(context):
    ...
    f = open(os.path.join(args.train_dir, 'response.txt'), "r", encoding="utf-8")
    while True:
        line = f.readline()
        if not line: break
    ...
    f = open(os.path.join(args.train_dir, 'inference.txt'), "w", encoding="utf-8")
    for response in response_list:
        if flag == False:
            line = "1" + "\t" + "participant 1: " + context + "\t" + response
            f.write(line)
            flag = True
        else:
            line = "0" + "\t" + "participant 1: " + context + "\t" + response
            f.write(line)
    f.close()
...
def inference_running_model(dataloader,  response_list, test=False):
    model.eval()
    ...
    print(response_list[logits.argmax(-1).item()])
...
if __name__ == '__main__':
    ...
    parser.add_argument("--architecture", default='poly', type=str, help='[poly, bi, cross]') # required=True
    ...
    MODEL_CLASSES = {
        'bert': (AutoConfig, AutoTokenizer, AutoModel),
    }
    ConfigClass, TokenizerClass, BertModelClass = MODEL_CLASSES[args.model_type]
    print(os.path.join(args.bert_model, "vocab.txt"))
    ...
    ## build BERT encoder
    ########################################
    bert_config = ConfigClass.from_pretrained(args.bert_model)
    ...
    if args.inference:
        while True:
            context = input("문장을 입력하세요 : ")
            if context == "end":
                break
            response_list = inference(context)
            model.load_state_dict(torch.load(state_save_path))
            inference_dataset = SelectionDataset(os.path.join(args.train_dir, 'inference.txt'),
                                                                    context_transform, response_transform, concat_transform, sample_cnt=None, mode=args.architecture)
            inference_dataloader = DataLoader(inference_dataset, batch_size=args.eval_batch_size, collate_fn=val_dataset.batchify_join_str, shuffle=False, num_workers=0)
            test_result = inference_running_model(inference_dataloader, response_list, test=True)
        exit()
